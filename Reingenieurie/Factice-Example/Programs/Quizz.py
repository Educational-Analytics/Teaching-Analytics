...
def Level_Percent(type, level, attempts):
    ...
    return learners, avg_learners_attempts, learners_success, learners_failure, learners_success_prct, success_attempts, failure_attempts, true_prct
...
def TyLvl_ScsFlr(type, rand_num, attempts):
    ...
    learners, avg_learners_attempts, learners_success, learners_failure, learners_success_prct, success_attempts, failure_attempts, true_prct  = Level_Percent(type, correct_level, attempts)

    #Return all the variables defined through the two previous functions
    return predict_level, correct_level, learners, avg_learners_attempts, learners_success, learners_failure, learners_success_prct, success_attempts, failure_attempts, true_prct
...
depts_type = ['Economie', 'Histoire', 'Droit', 'Management', 'Art', 'Sociologie', 'Mathématique', 'Ingenieurie', 'Philosophie', 'Informatique', 'Langues'] #Define a list of Departements
...
quiztype_percent = ['QCU'] * 45 + ['QCM'] * 35 + ['QROC'] * 15 + ['QO'] * 5
                ...
                for chap in range(0, chapter_quiz):
                    ...
                    if quiz_ID in lst_quiz_ID:
                        if lst_levelcorrect[ins[-1]] == "Adapte":
                            ...
                            learners, avg_learners_attempts, learners_success, learners_failure, learners_success_prct, success_attempts, failure_attempts, true_prct  = Level_Percent(type_quiz, correct_level, num_attempts)    
                        else:
                            predict_level, correct_level, learners, avg_learners_attempts, learners_success, learners_failure, learners_success_prct, success_attempts, failure_attempts, true_prct = TyLvl_ScsFlr(type_quiz, rand_num, num_attempts)                     
